...
3. **Gateway (BFF)** — the only component holding model, AWX, Wazuh, Proxmox, GitHub and Supabase service credentials.
...
| T9 | Resource exhaustion / cost blow-out | Per-run CPU/RAM/disk/wall-clock caps, max concurrent runs, per-role rate limits, request timeouts, model token/cost budget recorded in `ai_model_usage` with a hard monthly ceiling |
...
| T12 | Loss of audit integrity | `audit_events` append-only (no update/delete grant to the gateway role); events forwarded to Wazuh; run timeline immutable once terminal |
...
| T14 | Supply-chain compromise of OpenHands or dependencies | Pinned image digests (no `:latest`), lockfiles, dependency and container scanning in CI, upgrades reviewed rather than automatic |
...
runs with least-privilege service accounts, isolated egress and Wazuh alerting on
anomalous tool volume.
